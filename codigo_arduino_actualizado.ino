...
#define UMBRAL_VOLT_CRITICO 135  // Voltaje crítico, considerar sin energía
#define UMBRAL_VOLT_MINIMO 200   // Voltaje mínimo normal para forzar 220V
...
const String REGISTER_IP_URL = "http://192.168.2.178/Tesina/public/register_ip";
...
// Función para registrar la IP del ESP32 en el servidor
void registerESP32IP() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("⚠️ WiFi no conectado, no se puede registrar IP");
    return;
  }

  HTTPClient http;
  http.begin(REGISTER_IP_URL);
  http.addHeader("Content-Type", "application/json");

  String postData = "{\"mac_address\":\"" + macAddress + "\",\"ip_address\":\"" + WiFi.localIP().toString() + "\"}";

  Serial.println("📡 Registrando IP del ESP32 en el servidor...");
  Serial.println("📋 JSON: " + postData);

  int httpResponseCode = http.POST(postData);

  if (httpResponseCode > 0) {
    Serial.printf("✅ IP registrada - HTTP Response code: %d\n", httpResponseCode);
    String response = http.getString();
    Serial.println("📩 Respuesta del servidor: " + response);
  } else {
    Serial.printf("❌ Error registrando IP: %s\n", http.errorToString(httpResponseCode).c_str());
  }
...
void setupAPMode() {
  ...
  server.on("/rele", HTTP_GET, []() {
    if (server.hasArg("foco") && server.hasArg("estado")) {
      ...
      if (foco == 1) {
        digitalWrite(RELE_NO_ESENCIAL, encender ? HIGH : LOW);
        estado_focos[1] = encender;
      }
      if (foco == 2) {
        digitalWrite(RELE_ESENCIAL, encender ? HIGH : LOW);
        estado_focos[2] = encender;
      }
      
      Serial.printf("🎛️ Control manual: Foco %d %s\n", foco, encender ? "ENCENDIDO" : "APAGADO");
      server.send(200, "text/plain", "OK");
    }
  });

  server.on("/estado", HTTP_GET, []() {
    String json = "{\"foco1\":" + String(estado_focos[1] ? "true" : "false") +
                  ",\"foco2\":" + String(estado_focos[2] ? "true" : "false") + "}";
...
void setup() {
  ...
  digitalWrite(RELE_NO_ESENCIAL, HIGH); // Inicialmente apagado (lógica invertida)
  digitalWrite(RELE_ESENCIAL, HIGH);    // Inicialmente apagado (lógica invertida)
  ...
  if (!isConfigured) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // Registrar la IP del ESP32 en el servidor
      registerESP32IP();
    } else {
...
void loop() {
  ...
  if (voltaje < UMBRAL_VOLT_CRITICO) {
    voltaje = 0;
    Serial.println("🚫 SIN ENERGIA EN EL SISTEMA");
  } else if (voltaje < UMBRAL_VOLT_MINIMO) {
    Serial.println("⚠️ Voltaje bajo, se ajusta a 220V para medición");
    voltaje = 220;
  }

  float potencia = voltaje * corriente;

  // Energía consumida en este intervalo (Wh)
  float energia_intervalo = potencia * (INTERVALO_SEGUNDOS / 3600.0);
  ...
  Serial.printf("Potencia instantánea: %.2f W\n", potencia);
  Serial.printf("Energía acumulada: %.6f kWh\n", kwh_acumulado);
  Serial.println("-------------------------------");

  // --- Control automático de relés por consumo ---
  if (kwh_acumulado >= UMBRAL_CONSUMO_KWH) {
    Serial.println("⚠️ Límite de consumo alcanzado: línea NO esencial desconectada.");
    Serial.println("✅ Línea esencial sigue funcionando normalmente.");
    digitalWrite(RELE_NO_ESENCIAL, LOW);  // Corta luces o línea no esencial
    digitalWrite(RELE_ESENCIAL, LOW);     // Mantiene electrodomésticos
    estado_focos[1] = false;  // Actualizar estado para API
    estado_focos[2] = true;   // Línea esencial sigue funcionando
  } else {
    digitalWrite(RELE_NO_ESENCIAL, HIGH);
    digitalWrite(RELE_ESENCIAL, HIGH);
    Serial.println("✅ Consumo dentro del límite: ambas líneas activas.");
    estado_focos[1] = true;   // Actualizar estado para API
    estado_focos[2] = true;   // Actualizar estado para API
  }
