<?php
/**
 * Script de prueba para verificar el funcionamiento del modal de alertas
 * Ejecutar desde la raíz del proyecto: php test_modal_alertas.php
 */
...
echo "🔧 PROBANDO MODAL DE ALERTAS\n";
echo "============================\n\n";
...
echo "📊 Probando getLatestDataByDevice...\n";
...
try {
    ...
    if ($data['success']) {
        echo "✅ getLatestDataByDevice funcionando:\n";
        ...
        if ($data['data']['kwh_acumulado'] > $data['limite_consumo']) {
            ...
            echo "   - Porcentaje: " . (($data['data']['kwh_acumulado'] / $data['limite_consumo']) * 100) . "%\n";
        } else {
...
try {
    ...
    if ($data['success']) {
        echo "✅ getCortesPendientes funcionando: " . count($data['cortes']) . " cortes pendientes\n";
...
echo "\n🔍 Verificando configuración de límites...\n";
...
echo "✅ Modal de alertas listo para mostrar datos correctos\n";
